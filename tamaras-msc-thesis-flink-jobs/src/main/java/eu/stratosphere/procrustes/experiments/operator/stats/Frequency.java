...
public class Frequency {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataSet<Tuple2<Integer, Integer>> heavyHitters = env.readTextFile(inputPath).flatMap(new CountInt(configOperatorStatistics()));
        Collection collection = heavyHitters.groupBy(0).sum(1).filter(new CountGreaterThan(minFrequency)).collect();
    ...
    private static boolean parseParameters(String[] args) {
        ...
        if (args.length > 4 && args.length <= 5) {
            ...
            minFrequency = Math.round(cardinality * fraction);
